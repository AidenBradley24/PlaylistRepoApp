...
namespace PlaylistRepoAPI.Controllers
{
	...
	public class DataController(PlayRepoDbContext db) : ControllerBase
	{
		...
		#region Add and Update requests
		...
		[HttpPatch("playlists/{id}/media")]
		public IActionResult PatchPlaylistMedias([FromBody] MediaDTO.PatchElement patch, [FromRoute] int id)
		{
			var playlist = db.Playlists.Find(id);
			if (playlist == null) return NotFound();
			...
			try
			{
				records = playlist.AllEntries(db.Medias).EvaluateUserQuery(patch.UserQuery);
			}
			catch (InvalidUserQueryException ex)
			{
				return BadRequest($"Invalid user query: {ex.Message}");
		...
		[HttpPatch("remotes/{id}/media")]
		public IActionResult PatchRemoteMedias([FromBody] MediaDTO.PatchElement patch, [FromRoute] int id)
		{
			var playlist = db.RemotePlaylists.Find(id);
			if (playlist == null) return NotFound();
		...
		public IActionResult DeletePlaylistMedias([FromRoute] int id, [FromHeader] string query = "", [FromHeader] bool alsoDeleteFile = false)
		{
			...
			try
			{
				records = playlist.AllEntries(db.Medias).EvaluateUserQuery(query);
			}
			catch (InvalidUserQueryException ex)
			{
				return BadRequest($"Invalid user query: {ex.Message}");
